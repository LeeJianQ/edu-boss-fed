<template>
  <div v-if="!isLoading" class="create-or-update">
    <el-card>
      <div slot="header">
        <el-steps :active="activeStep" simple>
          <el-step
            ...
            :key="index"
            @click.native="activeStep = index" :title="item.title" :icon="item.icon"></el-step>
        </el-steps>
      </div>
        ...
        <div v-show="activeStep === 0">
          ...
          <el-form-item label="课程描述">
            <el-input
              ...
              placeholder="概述1"
            ></el-input>
            <el-input placeholder="概述2" v-model="course.previewSecondField" type="textarea"></el-input>
          </el-form-item>
          ...
          <el-form-item label="课程排序">
            <el-input-number v-model="course.sortNum" label="文字描述"></el-input-number>
          </el-form-item>
        </div>
        <div v-show="activeStep === 1">
          <el-form-item label="课程封面">
            <!-- upload 上传文件组件，它支持自动上传，只需要把上传的参数配置一下就可以 -->
            <!--
              1. 组件需要根据绑定的数据进行图片预览
              2. 组件需要把上传成功的图片地址同步到绑定的数据中
              v-modle的本质还是父子组件通信
              1. 它会给子组件传递一个名字叫做 value 的数据 (props)
